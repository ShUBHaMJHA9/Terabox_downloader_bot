...
0%""",
        ...
        async def on_progress(progress):
            ...
            try:
                # Use helper for nicer formatting
                text = render_progress_text(
                    title="Download Progress",
                    percent=progress.progress_percent,
                    downloaded=progress.downloaded,
                    total=progress.total,
                    speed_bytes=progress.speed * 1024 * 1024 if hasattr(progress, 'speed') and progress.speed else 0.0,
        ...
        logger.info(f"[{download_id}] Attempting aria2c...")
        ...
        async def upload_progress(current, total, *args):
            ...
            try:
                ...
                # Update more smoothly but avoid spamming
        ...
        # Backup uploaded video to database channel if configured (server-side copy)
        try:
            if settings.database_channel and settings.database_channel != chat_id:
                # Use server-side copy to avoid re-uploading the file
                asyncio.create_task(
                    uploader.copy_message_to_channel(
                        from_chat_id=chat_id,
                        message_id=upload_msg.id,
                        channel_id=settings.database_channel
                    )
                )
                logger.info(f"[{download_id}] Scheduled server-side copy to database channel {settings.database_channel}")
        except Exception as e:
            logger.warning(f"[{download_id}] Failed to schedule backup copy: {e}")
...
def register_download_handlers(app: Client):
    ...
    async def text_handler(client: Client, message: Message):
        ...
        logger.info(f"[DownloadHandler] Received private message from {message.from_user.id}: {message.text[:120]}")
